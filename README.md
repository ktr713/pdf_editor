...
PDFを開き、「文字追加」または「画像追加」で要素を中央へ配置します。配置・編集した要素を右クリックして「すべてのページに適用」を選ぶと、同じ位置と設定で他ページへ複製できます。「ページ番号追加」では現在ページに番号を挿入し、位置を決めてから右クリックで全ページへ連番を展開できます。要素はドラッグ移動できます。Ctrl+Z/Ctrl+Yで取り消し・やり直し、Ctrl+Sで上書き保存できます。上書き時は同じフォルダーの一時PDFへ保存した後、元ファイルを置換します。
