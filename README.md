...
PDFを開き、「文字追加」または「画像追加」で要素を中央へ配置します。要素はドラッグ移動できます。Ctrl+Z/Ctrl+Yで取り消し・やり直し、Ctrl+Sで上書き保存できます。上書き時は同じフォルダーの一時PDFへ保存した後、元ファイルを置換します。
...
- ページ番号、フッター、全ページ・範囲適用は Phase 4 の対象です。
